...
class CommandManager extends Map {
    ...
    add(...commands) {
        for (const command of commands) {
            ...
            command.name = command.name ?? (0, node_crypto_1.randomUUID)().slice(0, 13).toUpperCase();
            if (!command.code) {
                BDJSLog_1.BDJSLog.error(`"${command.name}" can't be loaded!${[`|-> at: ${command.name}`].join('\n')}`);
                continue;
            }
            if (!tslib_1.__classPrivateFieldGet(this, _CommandManager_instances, "m", _CommandManager_validateType).call(this, command.type)) {
    ...
    async load(dir, providing_cwd = false, log = false) {
        ...
        for (const file of files) {
            ...
            if (stat.isDirectory()) {
                await this.load((0, node_path_1.join)(dir, file), providing_cwd, false);
                continue;
            }
            const command = require((0, node_path_1.join)(root, dir, file));
            if (Array.isArray(command)) {
                for (const cmd of command) {
                    if (!('code' in cmd)) {
                        BDJSLog_1.BDJSLog.error(`"${file}" can't be loaded!${[`|-> at: ${(0, node_path_1.join)(root, dir, file)}`].join('\n')}`);
                        continue;
                    }
                    if (!tslib_1.__classPrivateFieldGet(this, _CommandManager_instances, "m", _CommandManager_validateType).call(this, cmd.type)) {
                        BDJSLog_1.BDJSLog.error([
                            `Invalid command type "${cmd.type}" provided`,
                            `|-> at: ${(0, node_path_1.join)(root, dir, file)}`
                        ].join('\n'));
                        continue;
                    }
                    // Ensure command name.
                    cmd.name = cmd.name ?? (0, node_crypto_1.randomUUID)().slice(0, 13).toUpperCase();
                    // Assign command path.
                    cmd._path_ = (0, node_path_1.join)(root, dir, file);
                ...
                command.name = command.name ?? (0, node_crypto_1.randomUUID)().slice(0, 13).toUpperCase();
                // Assign command path.
                command._path_ = (0, node_path_1.join)(root, dir, file);
                this.set(command.name, command);
                tslib_1.__classPrivateFieldGet(this, _CommandManager_commandStatus, "f").push({
                    name: command.name,
                    path: command._path_,
            ...
            delete require.cache[require.resolve((0, node_path_1.join)(root, dir, file))];
        }
        if (log)
...
_CommandManager_cwd = new WeakMap(), _CommandManager_directory = new WeakMap(), _CommandManager_commandStatus = new WeakMap(), _CommandManager_instances = new WeakSet(), _CommandManager_validateType = function _CommandManager_validateType(type) {
    ...
    table
        .setHeading('Name', 'Type', 'Status', 'Path')
