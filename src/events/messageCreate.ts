...
export default new BaseEvent<[Message<true>]>({
	...
	async listener(bot, message) {
		...
		const unprefixed = Array.from(bot.commands.values()).filter(
			command => command.type === 'unprefixed'
		)
		const prefixed = Array.from(bot.commands.values()).filter(
			command => command.type === 'prefixed'
		)
		const always = Array.from(bot.commands.values()).filter(
			command => command.type === 'always'
		)
