...
export default new BaseEvent({
	...
	async listener(bot) {
		...
		const npmdata = (await result.body.json()) as {
		...
		if (autoUpdate && fetchedVersion !== currentVersion) {
			const res = exec('npm i bdjs@latest', error => {
				if (error) {
					BDJSLog.error(
						['AutoUpdate Error', JSON.stringify(error, null, 4)].join('\n')
					)
				} else {
					BDJSLog.info(
						['Updated successfully', 'Please reboot the process.'].join('\n')
					)
					process.exit()
				}
			})
