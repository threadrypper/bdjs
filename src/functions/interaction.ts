...
export default new BaseFunction({
	...
	parameters: [
		{
			name: 'Option Type',
			description:
				'Option type to be retrieved. (between "customID" | "focusedOption" | "menuOption" | "modalComponent" | "slashOption" | "subCommand" | "subCommandGroup")',
			required: true,
			resolver: 'String',
			value: 'none'
		},
	...
	code: async (d, [interactionType, ...options]) => {
		...
		switch (interactionType.toLowerCase()) {
			case 'customid': {
				if (
					...
					throw new d.error(
						...
						'disallowed',
						'Custom ID',
						'anyInteraction and componentInteraction'
					)
				return int.customId
			}
			case 'focusedoption': {
				if (
					!['anyInteraction', 'autocompleteInteraction'].includes(
						d.commandType
					) ||
					!int.isAutocomplete()
				)
			...
			case 'menuoption': {
				if (
					!['anyInteraction', 'selectMenuInteraction'].includes(
						d.commandType
					) ||
					!int.isAnySelectMenu()
				)
					throw new d.error(
						...
						'disallowed',
						'menuOption',
						'anyInteraction and selectMenuInteraction'
					)
				if (
					options[0] &&
			...
			case 'modalcomponent': {
				if (
					!['anyInteraction', 'modalInteraction'].includes(d.commandType) ||
					!int.isModalSubmit()
				)
					throw new d.error(
						d,
						'disallowed',
						'modalComponent',
						'anyInteraction and modalInteraction'
					)
			...
			case 'slashoption': {
				if (
					!['anyInteraction', 'commandInteraction'].includes(d.commandType) ||
