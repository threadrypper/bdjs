			...
			data.functions
				.add({
					...
					async code(t, [bitrate]) {
						if (channelOptions.type !== ChannelType.GuildVoice)
							throw new t.error(
								t,
								'disallowed',
								t.function!.name,
				...
				.add({
					...
					async code(t, [target, ...permissions]) {
						...
						if (target !== guild.id) {
							role =
								(await d.util.getRole(target, guild)) ??
								(await d.util.getMember(target, guild))
							if (!role)
								throw new t.error(
									t,
									'custom',
						...
						;(
							channelOptions.permissionOverwrites as OverwriteResolvable[]
						).push({
							id: target,
							allow,
							deny
				...
				.add({
					name: 'setUserLimit',
					description:
						'Set the limit of users that can connect to the channel.',
					async code(t, [limit]) {
						if (channelOptions.type !== ChannelType.GuildVoice)
							throw new t.error(
								t,
