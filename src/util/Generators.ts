...
class FunctionInfo {
	...
	async toMD() {
		const args =
			(this.extraOptions.params?.length ?? 0) > 0
				? `## Parameters\n${this.getParamTable()}`
				: undefined
		const special =
...
export class Generators {
	...
	static async loadFunctions(input: string, input_providing_cwd = false) {
		const files = (
			await readdir(join(input_providing_cwd ? '' : process.cwd(), input))
	...
	static async documentFunctions(input: string, output: string) {
		...
		await mkdir(join(input_providing_cwd ? '' : process.cwd(), ...paths), {
			recursive: true
		})

		const accumulated = await Generators.loadFunctions(
			input,
			input_providing_cwd
		)
		accumulated &&
			Generators.getSideBar(
				accumulated,
				join(
	...
	static async getFunctionSchema(accumulated: FunctionInfo[], output: string) {
		...
		for (const func of accumulated) data.push(JSON.parse(JSON.stringify(func)))

		writeFile(
			join(output, 'FUNCTION.SCHEMA.json'),
	...
	static async getEventTable(output: string) {
		const files = (
			await readdir(join(__dirname.replace('util', 'events')))
	...
	static async getPropertiesTable(output: string) {
		const properties = require('../util/Properties').default as Record<
			string,
