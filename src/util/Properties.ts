...
function defineProperties<T>(
	data: Record<
		...
		{
			description: string
			code: (a: T) => string | boolean | number | null | undefined
		}
	>
) {
	return data
...
export default {
	...
	Guild: defineProperties<Guild>({
		...
		defaultmessagenotification: {
			description:
				"Returns this guild's default message notifications setting.",
	...
	Thread: defineProperties<AnyThreadChannel>({
		...
		members: {
			...
			code: t =>
				Array.from(t.guildMembers.values())
					.map(m => m.id)
					.join(',')
		},
		membercount: {
