 ...
 */
type GetPayload<T> = T extends Message
	? MessageCreateOptions
	: T extends ChatInputCommandInteraction
		? InteractionReplyOptions & { fetchReply?: boolean }
		: T extends MessageComponentInteraction
			? InteractionReplyOptions
			: MessagePayload

type GetEditPayload<T> = T extends Message
	? MessageEditOptions
	: T extends ChatInputCommandInteraction
...
function isValidInstance(
	data: TextChannel | ThreadChannel | StageChannel | NewsChannel | User
) {
	...
	async send(payload: GetPayload<T> | string) {
		if (this.raw instanceof ChatInputCommandInteraction) {
			const reply = await this.raw.reply(
				payload as InteractionReplyOptions & { fetchReply?: boolean }
			)
			if (
